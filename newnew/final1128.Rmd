...
getwd()
setwd("C://Users//86084//Desktop/wpi/2020 fall/DS502/final project/dataset")
...
for (i in 1:k){
  ...
  lm_ori_train[i] = mean(abs((training_dataset$SalePrice)- (lm_ori_pred1))/(training_dataset$SalePrice)<=0.05)
...
for (i in 1:k){
  ...
  lm_log_train[i] = mean(abs(exp(training_dataset$SalePrice)- (lm_log_pred1))/exp(training_dataset$SalePrice)<=0.05)
}
...
for (i in 1:k){
  ...
  lm_sqrt_train[i] = mean(abs((training_dataset$SalePrice)^4- (lm_sqrt_pred1))/(training_dataset$SalePrice)^4<=0.05)
...
lm_accuracy_df = data.frame(lm = c('train accuracy','test accuracy'),ori_accuracy
                            =c(lm_ori_aver_train,lm_ori_aver_accuracy),
                            log_accuracy = c(lm_log_aver_train,lm_log_aver_accuracy),
                            sqrt_accuracy = c(lm_sqrt_aver_train,lm_sqrt_aver_accuracy))
lm_accuracy_df
#lm_pred  = exp(lm_pred)
...
for ( i in 1:k){
  ...
  Ridge_ori_train[i] = mean(abs((y_train) - (Ridge.ori.Pred1))/(y_train) <=0.05)
...
for ( i in 1:k){
  ...
  Ridge_log_train[i] = mean(abs(exp(y_train) - (Ridge.log.Pred1))/exp(y_train) <=0.05)
...
for ( i in 1:k){
  ...
  Ridge_sqrt_train[i] = mean(abs((y_train)^4 - (Ridge.sqrt.Pred1))/(y_train)^4 <=0.05)
...
for ( i in 1:k){
  ...
  Lasso_ori_train[i] = mean(abs((y_train) - (Lasso.ori.Pred1))/(y_train) <=0.05)
...
Lasso_ori_aver_coef = (Lasso_ori_coef/5)[1:215,]
...
for ( i in 1:k){
  ...
  Lasso_log_train[i] = mean(abs(exp(y_train) - (Lasso.log.Pred1))/exp(y_train) <=0.05)
...
Lasso_log_aver_coef = (Lasso_log_coef/5)[1:215,]
...
for ( i in 1:k){
  ...
  Lasso_sqrt_train[i] = mean(abs((y_train)^4 - (Lasso.sqrt.Pred1))/(y_train)^4 <=0.05)
...
Lasso_sqrt_aver_coef = (Lasso_sqrt_coef/5)[1:215,]
